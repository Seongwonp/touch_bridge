...
> 이 문서는 GRBL XYZ 기반 v1 계약이다. FIT0482 X/Y + 스위치봇 v2 전환 계획은
> `XY_SWITCHBOT_SOFTWARE_PLAN.md`를 따른다. v2 펌웨어 계약이 확정되기 전까지
> v1 런타임 호환을 유지한다.
...
4. 개발자/수동 조이스틱 이동은 `$J` 대신 `G91` -> `G1 <axis><value> F<feed>` -> `G90` 순서의 안정 경로를 사용한다.
