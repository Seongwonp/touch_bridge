...
### 개발보고서 — SW 서술만 비전공자용으로 개정 (HW 유지)
사용자 요구를 재확인해 방향을 정정했다. 앞서 만든 "HW 제거본"은 요구와 달랐고,
실제 요구는 **원본을 그대로 두고 SW 설명 문장만 쉽게 고치는 것**이었다.

- 원본 `2026년_한이음_드림업_개발보고서_터치브릿지_SW최신화.docx` 기준, 문단 48개와
  요약본·전체 기능 목록(S/W 행)·S/W 주요 기능 표를 비전공자용 표현으로 개정.
  전문 용어를 풀어 쓰고 단위를 설명(예: 6~8ms → 6~8밀리초(1000분의 6~8초)),
  화살표 흐름은 번호 단계로 분해.
- **HW는 전부 보존** — CoreXY 역기구학, GRBL, V홈 레일, MR63ZZ, 도웰핀 분할 설계,
  전력 관리, H/W 주요 기능 표, H/W 구성장비, 기구 설계 최적화, 부착형 플로터 항목.
  서식·표 구조·사진·쪽 구성(14쪽)도 원본 그대로.
- 참고문헌: `(확인일: ...)` 표기 전량 제거. Toucha11y·BrushLens의 제목과 제1저자가
  실제 논문과 달라 원문 확인 후 정정(Guo→Li, J. / Zhao→Liang, C.).
  Android Accessibility Codelab은 안드로이드 XML 실습 튜토리얼이라 Flutter 앱 근거로
  부적합하고 [5]와 중복이라 삭제. [1]~[12]로 재정렬, 본문 인용 일치.
- run이 둘로 쪼개진 문단 2건은 서식이 동일함을 확인한 뒤에만 병합하도록 가드를 두었다.
- 검증: Word 정상 열림, 14쪽 전부 렌더 검수. HW 키워드 잔존 확인 스크립트 통과.

### (중간 산출물) SW 전용 제출본 — 요구 오해로 폐기
...
`docs/submission/SW_개발보고서_수정메모.md`. 양식·서식·HW 내용은 그대로 유지(14쪽).

- **사실과 다른 수치 정정** — "코드 분석에서 보고된 문제는 0건"을
  "오류 0건·경고 4건(2026-09-07 기준)"으로 수정. 경고의 성격과 처리 계획도 명시.
  자동 점검 318개 전부 통과는 위 회귀 수정 후 실측값.
- **사진 설명 정정** — 1차 시제품 실물 사진을 "전체 콘셉트 이미지"로 설명하고 있어
  실제 내용에 맞게 수정. 본문의 CoreXY 재설계 서술과도 어긋나던 부분.
- **참고문헌 16개 → 14개** — 링크를 직접 확인해 두 건만 삭제.
  - RNIB "Accessibility for product design": `old-version` 경로로 본문이 열리지 않으며
    이 자료만으로 뒷받침한 주장도 없음.
  - Google "Android Accessibility Codelab": 안드로이드 XML 앱 실습 튜토리얼로
    Flutter 앱의 근거로 부적합하고 남은 Android 접근성 원칙 문서와 중복.
  - 나머지 14건은 생존·내용 확인 후 유지. 번호를 [1]~[14]로 재정렬하고 본문 인용도 일치시킴.
    빠진 번호 없음, 14건 전부 본문에서 인용됨.
- **범위 인용 해소** — "[8]~[16]" 식 일괄 인용을 항목별 구체 인용으로 분리.
- **비전공자용 표현** — 부착형 플로터/매핑/조이스틱 보정/ms 단위 등을 풀어 씀.
- 검증: Word 정상 열림(복구 창 없음), 14쪽 전부 렌더 검수, 표·이미지·번호 이상 없음.
...
- 제목을 짧게 정리하고 화면 설명을 실제 행동 중심으로 수정. 예: “입력·출력·처리 명세” 대신 “기기를 고른다”, “잘못 알아들으면 다시 말한다”.
- 내장 image_gen으로 설명 삽화 3개 제작, 11쪽 음성 조작 장면·13쪽 화면읽기와 큰 글씨·23쪽 사진 위치 보정에 삽입. 실제 앱 캡처는 유지. 장치 삽화는 CoreXY 방향을 반영한 개념 설명이며 제작 도면·실물 증적이 아님.
- 프레젠테이션 스킬에 따라 원본 양식과 편집 가능한 표·기존 흐름도를 보존. 그림과 본문 간격, 출처 번호의 줄바꿈을 보완.
- 최종 `TouchBridge_SW_제작설계서_그림설명_v4.pptx` 35쪽을 다시 렌더하고 모두 검수. 패키지 검사 통과, 레이아웃 finding 0. 원본 장식 경계 경고는 유지. PowerPoint 직접 실행 검증은 하지 않음.
- 그림 파일 및 최종 생성 프롬프트는 `docs/submission/assets/SW_그림_제작기록.md`에 기록. 별도 그림설명 원고도 저장.
- 이번 작업은 문서·그림 변경만 수행. 앱 코드와 기존 파일은 보존하고 Flutter 테스트, 커밋, 푸시는 하지 않음.

## 2026-09-07 — 제작설계서 SW 쉬운 설명·연구 연결 보강

- 제공된 35쪽 PPTX 양식을 유지한 SW 검토용 작성본 `TouchBridge_SW_제작설계서_쉬운설명_v2.pptx` 생성. 원본은 변경하지 않음.
- 사용자 연구를 3·4쪽, 연구와 접근성 설계·확인 항목의 연결을 13쪽, 출처를 35쪽과 발표자 노트에 표시. 자체 설문·인터뷰 결과로 오인하지 않도록 구분.
- 주요 SW 기능 12개 요구사항, 화면·메뉴·데이터·알고리즘·실제 핵심 코드와 쉬운 설명 작성. 다시 듣기·명령 반복·상태 질문·연습·전화 도움도 포함.
- CoreXY를 사용자 확인 방향으로 반영하되 기구·회로는 HW 담당 범위로 제외. 앱과 장치의 응답·이동·정지 통합 시험은 미확인으로 명시.
- 저장소 316개 시험·기존 경고 4건과 제공 보고서 318개·분석 0건의 차이를 숨기지 않고 버전 대조 항목으로 기록. QR 비활성, 실제 가전 상태와 앱 타이머의 차이도 명시.
- 쪽별 원고와 `SW_작성범위와_확인사항.md` 생성. 기존 개발보고서는 요약이며 모든 코드 분기를 포함한 상세 명세가 아니라는 작성 범위 설명.
- 프레젠테이션 스킬에 따라 편집 가능한 표·흐름도를 사용하고 최종 35쪽 모두 이미지로 검수. 35쪽 하단 겹침 수정. 패키지 검사 통과, 레이아웃 finding 0; 원본 모서리 장식의 경계 밖 경고 33건 유지. 실제 PowerPoint 실행 검증은 미실시.
- 앱 코드 변경 및 Flutter 테스트 재실행 없음. 문서 검증만 수행했으며 커밋·푸시하지 않음.

## 2026-09-07 — 개발보고서 SW 쉬운 설명 개정

- 사용자 제공 DOCX를 바탕으로 SW·배경 설명 69개 문단을 비전공자용으로 개정.
- 시각장애인·저시력자의 가전 사용 연구를 앞세우고 플랫폼 자료는 구현 참고로 분리. 관련 연구 서지 오류 수정.
- 실제 앱 캡처를 유지하고 가독성을 높임. CoreXY 사용 상황과 안내 방식 삽화 2개 추가, 개념도임을 명시.
- 원본 SHA256 불변 및 기존 패키지 보존 검증. Word 읽기 전용 변환 후 전체 페이지 이미지로 검수.
- 앱 코드 변경·테스트 재실행 없음. 318개 등 수치는 제공 문서의 기존 시험 기록으로 명시하며 당사자 사용 평가와 구분.
- 산출물과 편집 기록은 `docs/submission/`에 저장. 커밋·푸시는 수행하지 않음.

## 2026-09-06 — 보호자 위치 맞추기 UX·개발보고서 S/W 최신화

### 변경

- 수동 매핑을 개발자용 행·열 입력 화면에서 보호자용 3단계 흐름
  `위치 맞추기 → 눌러보기 → 저장`으로 재설계.
- 3×3 고정 격자를 제거하고 최대 9개의 버튼을 자유롭게 추가·선택해 실제
  X/Y 좌표를 저장하도록 변경. 진입 시 첫 버튼을 자동 선택하지 않아
  `1분` 등 특정 버튼이 고정된 것으로 오해하지 않게 함.
- 패널 그림에 등록 버튼과 터치봉의 앱 기준 현재 위치를 표시하고, 1/5/10mm
  이동 단위의 큰 조이스틱과 SwitchBot 눌러보기를 보호자 작업 문맥으로 제공.
  Z축·행/열·원점·간격 등 기술 설정은 고급 설정으로 분리.
- 버튼 추가 대화상자 종료 직후 컨트롤러를 폐기해 발생하던 Flutter
  `_dependents.isEmpty` 오류를 대화상자 제거 완료 뒤 폐기하도록 수정.
- 한이음 드림업 개발보고서의 S/W 개요·주요 기능·적용 기술·문제 해결·개발
  환경·검증 수치를 현재 구현과 동기화하고, Android Emulator에서 촬영한 홈,
  보호자 자유 배치 매핑, 기기 연결 화면을 삽입. 기존 H/W 내용과 문체는 유지.
- 보고서 표 행 분할·강제 페이지 나눔을 정리하고, 그림 11개 대체텍스트와
  표 9개 머리글 속성을 추가해 문서 접근성 자동 검사 0건을 달성.

### 검증

- `flutter analyze`: **No issues found**.
- `flutter test`: **318/318 통과**. 보호자 위치 맞추기 UI 및 버튼 추가
  대화상자 종료 회귀 테스트 2건 포함.
- 개발보고서 11쪽 전체 렌더링 육안 검수 완료. 잘린 글자·표 머리글 단독
  이월·의도하지 않은 큰 공백 없음.
- 문서 접근성 감사: high 0 / medium 0 / low 0.
- 실제 모터·ESP32·SwitchBot 종단간 동작과 TalkBack/VoiceOver 당사자 평가는
  별도 실기기 후속 검증으로 유지하며, 보고서에도 완료로 과장하지 않음.

---

## 2026-09-05 — 한이음 제작설계서 SW 초안 작성

- 안전성 보완 커밋 `84dec09`를 `origin/main`에 push 완료 후 작성.
- 제공된 35쪽 PPTX 양식의 4:3 비율·로고·제목 스타일을 유지한 별도 초안과
  쪽별 Markdown 원고를 `docs/submission/`에 생성. 제공 원본은 변경하지 않음.
- 요구사항·화면·구성도·저장 구조·알고리즘·실제 코드·테스트 근거를 반영.
  표와 흐름도는 편집 가능한 개체로 작성. 팀원 개인정보는 넣지 않음.
- 설문·인터뷰, 최신 릴리스 화면, 신규 하드웨어 실기 사진·측정은
  확보 전으로 명시. GRBL 일반 제어와 신규 모션 v2 검증 경로도 구분.
- 검증: 35쪽 렌더 확인, 패키지 무결성 통과, 레이아웃 finding 0.
  26쪽 표 너비를 보완해 native table overflow 경고 해소.
  남은 경고 33건은 원본 양식의 바깥쪽 모서리 장식이며 유지.
  실제 Microsoft PowerPoint 실행 검증은 미실시.
- 프레젠테이션 스킬에 따라 원본 스타일 보존·편집 가능성·렌더 검증 수행.
  이번 산출물은 제출 완료본이 아닌 검토용 초안이며, 작성 파일은 아직 미커밋.

## 2026-09-05 — 코드 리뷰 후 모션·매핑 안전성 보완

### 변경

- `calibrationInvalidated` 영속 플래그로 보정 무효화와 구버전 그리드를 구별.
  일반 버튼 실행·좌표 해석을 재보정 전 차단하고 수동 그리드 병합 시 플래그 보존.
- 모션 컨트롤러의 연결 구독을 수명 전체로 확장. 재연결 및 기기 변경 시
  기존 원점 확인을 재사용하지 않으며 화면 종료 시 구독 해제.
- 정지 요청 시 안전 세대를 변경해 진행 중 작업·이전 대기 작업·지연 응답을
  무효화. 공통 BLE 정지 경로와 연동하고 v2 전송 직전에도 세대를 검사.
- 이동/누름 응답 타임아웃과 프로토콜 오류는 별도 정지 요청을 시도하되
  실제 정지 성공으로 안내하지 않음. 취소된 앱 작업도 물리 멈춤과 구분.
- 운영 로그: `mapping.press.calibration_invalidated`, `motion.safety.invalidated`,
  `CANCELLED_V2`. 관련 모션 프로토콜·SW 계획·CLAUDE 문서 갱신.

### 검증

- 회귀 테스트 추가: 보정 무효화 JSON 왕복/전송 차단, 대기 중 재연결,
  기기별 홈 구분, 정지 후 지연 완료/대기 작업 폐기, 누름 중 정지,
  타임아웃 안내·정지 요청, 컨트롤러 구독 해제, 수동 병합 후 저장/로드,
  홈 대기 중 화면 종료. 총 10건 추가.
- `flutter test --no-pub`: **316/316 통과**. 1차 관련 회귀 실행도 69/69 통과.
- `git diff --check`: 통과.
- 환경: 누락된 `flutter_secure_storage` 캐시는 `pub get --enforce-lockfile`로
  복원(의존성 업그레이드 없음). OneDrive 테스트 산출물 접근 오류는
  `build/unit_test_assets`를 `build/unit_test_assets.review-backup-20260905`로
  보존한 뒤 새 생성물로 검증하여 해소. 소스/사용자 데이터 삭제 없음.
- 한글 경로에서 분석 서버 LSP 파싱 오류가 발생하여 같은 저장소를 가리키는
  `X:\touchbridge` 별칭으로 정적 분석 실행.
- `flutter analyze --no-pub`: 오류 0, 경고 4, info 0 (경고로 종료 코드 1).
  수정하지 않은 `appliance_config.dart:22`, `home_screen.dart:143`,
  `home_device_card.dart:105`, `device_management_screen.dart:204`의 동적
  `IconData` 관련 `non_const_argument_for_const_parameter` 경고가 남음.
  신규 수정 파일의 분석 지적은 없음. 분석 전체 통과/이슈 0건으로 표기하지 않음.
- ESP32·SwitchBot 실기 정지/재연결 검증은 미수행. 소프트웨어 테스트만으로
  물리 안전 완료를 주장하지 않는다.

### 후속 작업

- 사용자가 제공한 한이음드림업 제작설계서 PPTX의 SW 부분 작성 지원.
  이번 수정·검증을 먼저 마무리한 뒤 실제 구현/예정 기능을 구분해 작성한다.
...
## 2026-08-22 — [기능 A-2] 연습 모드

### 배경
- 시각장애인 사용자가 실제 가전 앞에서 처음 배우면 실수가 곧 오작동이다.
  이중 탭·비상 홀드·음성 명령을 **아무것도 실행되지 않는 환경**에서 반복 연습할
  경로가 필요했다(당사자 검증 계획 USER_VALIDATION_PLAN의 학습 곡선 대응).
  부수 효과: 하드웨어 없는 발표장에서 UX를 그대로 보여주는 시연 플랜 B.

### 구현 (`lib/screens/practice/practice_screen.dart`, 설정 > 연습 > 연습 모드)
- 레슨 1 **이중 탭**: 실제와 동일한 arm 패턴(20초 자동 해제 포함 — 해제도
  "실제 화면에서도 이렇게 취소됩니다"로 학습 기회화). 성공 횟수 카운트 + 칭찬 TTS.
- 레슨 2 **비상 정지 3초 홀드**: 실제 카운트다운 화면과 동일한 홀드+진행바.
  중간에 떼면 "3초 전에 손을 뗐어요" 교정 안내.
- 레슨 3 **음성 명령**: SpeechSessionService로 청취, **로컬 규칙만으로 해석**
  (백엔드/AI 미사용 — 연습은 네트워크에 의존하지 않는다). 이해한 명령을
  "실제라면 ○○ 버튼을 눌렀을 거예요"로 번역해 들려줌. STT 불가 환경은
  정직하게 "사용 불가" 표시.
- 모든 피드백 result 우선순위(스크린리더에서도 들림), 성공 배지 liveRegion.
- BLE/백엔드 코드 경로에 일절 접근하지 않음 — "아무것도 실제로 실행되지 않아요"
  약속이 구조적으로 보장됨.

### 테스트
- 위젯 테스트 5건: 3개 레슨+비실행 안내 표시 / 이중 탭 첫 탭 arm만·둘째 탭 성공 /
  20초 자동 해제 / 3초 홀드 완주 성공·중도 해제 실패 / STT 불가 정직 표시.
- FakeAsync에서 Ticker 첫 프레임이 기준점이라 홀드 애니메이션은 프레임 분할
  pump가 필요함을 확인(테스트 주석화).
- 검증: `flutter analyze` 신규 이슈 0건, `flutter test` 235/235 통과.

---

## 2026-08-22 — [기능 A-3] 상태 질의 음성 명령 ("얼마나 남았어?")

### 배경
- 화면을 볼 수 없는 사용자에게 '현재 상태를 언제든 물어볼 수 있다'는 것은 핵심
  안심 장치인데, 남은 시간은 카운트다운 화면의 로컬 상태라 다른 화면에서 답할
  방법이 없었다.

### 구현
- `RunStatusService` 신설 — 카운트다운 화면이 시작/틱/종료를 보고하는 전역 실행
  상태 저장소. **정직성 원칙**: 비상 정지·화면 이탈 등으로 앱이 남은 시간을 더
  추적하지 못하면 stop 처리해 "작동 중"이라 단언하지 않는다.
- `StatusIntent` 신설 — "얼마나 남았어/남은 시간/지금 뭐 해/진행 상황/돌아가고
  있어?" 등 질문형 표현만 좁게 매칭(명령형 "일시정지 상태로 해줘"와 충돌 방지).
  응답은 로컬 즉시 생성(AI/네트워크 무의존): 작동 중이면 "기기명 + N분 M초
  남았습니다", 아니면 선택 기기·허브 연결 상태 요약.
- 연결 지점 2곳: 음성 명령 화면(Help 다음 인터셉트 — 확인 대기 컨텍스트를 지우지
  않고 답한 뒤 재청취), 카운트다운 화면의 상시 청취("멈춰" 외에 상태 질의도 응답).

### 테스트
- 8건: 질의 판별(질문형 10종/명령형 오인 금지 4종), 응답 생성(작동 중 분·초 표현
  3종/미작동 시 기기·연결 요약 2종), RunStatusService 라이프사이클·정직성.
- 검증: `flutter analyze` 신규 이슈 0건, `flutter test` 243/243 통과.

---

## 2026-08-23 — [기능 B-1] "아까 그거 다시" 재실행 + 기기 별명

### 배경
- 가전 조작은 반복이 많다("어제 그 30초"). 매번 전체 명령을 다시 말하는 대신
  "아까 그거 다시" 한 마디로 재실행할 수 있어야 하고, 기기도 정식 명칭이 아니라
  평소 부르는 이름("우리집 세탁기")으로 부를 수 있어야 한다.

### 구현 — 재실행
- `RepeatIntent` 신설: "아까 그거/또 해줘/한 번 더 해줘" 등 재실행 요청 판별.
  `ReplayIntent`("다시 말해줘" = 안내 재생)와 **토큰 상호 배타성**을 테스트로
  고정 — 안내 재생 요청이 물리 명령 흐름으로 오라우팅되는 사고 방지.
- `LastCommandService` 신설: 전송 성공한 물리 명령(원본 파싱 데이터+기기+설명)을
  SharedPreferences에 영속화(앱 재시작 후에도 유효). 손상 저장값은 조용히 무시.
- 음성 화면: 전송 성공 4경로(IMMEDIATE/전자레인지/세탁기/에어컨)에서 기록,
  재실행 요청 시 기기 재등록 확인 → 활성화 → **기존 확인 질문 흐름 재사용**
  ("마지막 명령은 전자레인지, 30초 조리예요. 다시 실행할까요?" → 예/아니오).
  물리 동작이므로 확인 없이 실행하지 않는다.

### 구현 — 기기 별명
- `RegisteredVoiceDevice.aliases` + `VoiceDeviceResolver` 최장 일치 매칭
  (이름 "세탁기"보다 별명 "우리집 세탁기"가 먼저 매칭돼야 명령 텍스트에
  찌꺼기가 안 남는다). 서로 다른 기기가 이름/별명으로 함께 언급되면 되묻기 유지.
- 기기 관리 화면에 "음성 별명" 행 + 쉼표 구분 편집 다이얼로그 (보호자 설정).

### 테스트
- 11건: RepeatIntent 판별/오인 금지/Replay 상호 배타(회귀 고정),
  LastCommandService 왕복·재시작 복원·없음·손상 무시, 별명 매칭·최장 일치·
  다중 언급 되묻기·JSON 이물질 방어.
- 검증: `flutter analyze` 신규 이슈 0건, `flutter test` 254/254 통과.

---

## 2026-08-23 — [기능 B-2] 접근성 실험 지표 화면 + CSV 내보내기

### 배경
- README는 "설정 화면의 접근성 실험 지표 섹션"을 안내했지만 **실제 화면이
  존재하지 않았다**(리뷰의 문서-코드 불일치 유형). 또한 리뷰 지적대로
  (a) 중단(recordTaskAborted)이 저장되지 않아 완료율 해석이 모호했고,
  (b) 완료 시간이 음성/수동 모드별로 분리되지 않아 당사자 검증의 핵심 비교
  ("음성 vs 수동")가 불가능했다.

### 구현
- `AccessibilityExperimentService` 확장: 중단 카운트 집계·저장, 시작 모드 기억
  → 음성/수동 완료 수·완료 시간 분리 집계(+모드별 평균 getter). 비정상 순서
  (시작 없는 완료)는 분리 지표를 오염시키지 않음. reset은 신규 지표 포함.
- `buildExportCsv()`: 전 지표를 "이름,값" 2열 CSV로 생성(내보낸 시각 포함) —
  당사자 검증 실측 데이터 수집 창구.
- 설정 화면에 지표 섹션 신설(ListenableBuilder 실시간 갱신): 지표 요약 6행 +
  `지표 내보내기(CSV 복사)` (클립보드 복사, 의존성 추가 없음, result TTS 안내) +
  `지표 초기화` (확인 다이얼로그 — "내보내기를 먼저 했는지 확인" 경고).

### 테스트
- 5건: 모드별 분리 집계 / 중단 집계·영속화 / 비정상 순서 오염 방지 /
  CSV 전 지표 포함·2열 형식 검증 / reset 전체 초기화.
- 검증: `flutter analyze` 신규 이슈 0건, `flutter test` 259/259 통과.

---

## 2026-08-23 — [기능 B-3] 소리·진동 어휘 학습 (연습 모드 레슨 4)

### 배경
- 앱은 "전송 ≠ 확인"을 earcon·진동으로 구분해 전달하지만(멀티모달 원칙),
  각 신호의 뜻을 배울 곳이 없었다. 소음 환경에서 TTS를 놓쳤을 때 소리·진동이
  보조 채널이 되려면 어휘를 미리 익혀야 한다.

### 구현 (연습 모드 레슨 4)
- 4개 신호 카드: 받았어요(띵)/보냈어요/확인됐어요/실패했어요 — 탭하면 실제
  earcon+진동 재생 후 뜻을 TTS(result)로 설명.
- "보냈어요" 설명에 '아직 기기 확인 전이라 완료는 아니에요'를 명시 —
  전송≠확인 구분 자체를 학습 내용으로 만들고 테스트로 고정.

### 테스트
- 1건: 4개 카드 존재 + 전송≠확인 설명 문구 + 탭 재생 무예외.
  (ListView 지연 빌드 특성상 scrollUntilVisible 사용 — 테스트 주석화)

---

## 2026-09-04 — FIT0482 X/Y + 스위치봇 소프트웨어 M1~M5

### 배경

- 42각 NK1704S 3축 구조가 목표 제품 크기에 비해 커서 X/Y는 FIT0482 엔코더
  기어모터, Z는 스위치봇을 쓰는 방향으로 전환했다.
- 사진의 행/열 셀만으로 물리 버튼을 누르는 기존 경로는 기구 오차와 왜곡을
  설명하지 못하므로 실제 mm 좌표와 폐루프 위치 확인을 앱 계약에 반영했다.

### 구현

- M1~M2: 버튼별 정규화 중심/실제 mm 좌표, schema 마이그레이션 로그, Vision
  결과 범위·중복·신뢰도 검증, 패널 네 모서리 투영 보정과 안전 범위 검증.
- M3: `MotionController`/`PressActuator` 분리, ESP32 v2 명령·상태 계약,
  HOME 완료/오차 ±0.7mm/positionToken 게이트, 스톨·리미트·시간초과·통신 오류,
  우선순위 비상 정지 경로.
- M4: 전체 자동 누름을 없애고 한 점씩 `move_only` 또는 실제 누름을 확인 후
  실행. 고위험 버튼 추가 확인, 컨트롤러·보호자 실측 오차와 실패를 기기별
  최근 100건까지 저장.
- M5: 설치 화면 제목을 사용자 실행 화면과 구분. 촬영 직전 뒤/좌우/상하/
  수평·반사 네 단계 음성·진동 안내. 패널 모서리와 버튼 중심에 0.5% 단위
  방향 버튼 제공. 화면읽기 활성 시 단순 화면 이동만 앱 확인을 생략하고
  물리·삭제 확인은 유지.

### 운영 로그

- 촬영 안내: `mapping.camera_guide_step/completed/cancelled`
- 위치 조정: `mapping.calibration_corner_nudged`,
  `mapping.button_position_nudged`
- 화면읽기 확인 생략: `accessibility.navigation_confirmation_skipped`
- 기존 모션/검증 로그와 함께 commandId, 실패 상태, 도착 오차, 실측 오차를
  추적한다.

### 검증

- `flutter analyze --no-pub`: 이슈 0건.
- `flutter test`: 306/306 통과.
- `backend` `python -m unittest test_validation -v`: 3/3 통과.
- 실기 모터 PID/ACK와 전맹·저시력 분리 과업 평가는 문서화만 완료했으며 아직
  실행하지 않았다. 완료로 과장하지 않는다.

### 문서·출처 감사

- README 패키지 버전을 `pubspec.yaml`과 동기화하고 자동 테스트 수를 306개로
  갱신했다.
- `SOURCE_AUDIT_2026-09-04.md`를 추가해 W3C, Android, Apple, Flutter,
  DFRobot, 법령, 정부 조사, 학술 원 연구의 출처 등급·적용 범위·한계를 기록했다.
- WCAG2Mobile을 규범 표준처럼 보이게 하던 표현과 장애인차별금지법이 특정
  음성명령 UI를 직접 의무화한다는 과도한 표현을 교정했다.
- README의 77.1% 통계는 키오스크 조사 응답 범위로 한정하고, 학술 연구 결과를
  Touch Bridge 자체 성능처럼 읽히지 않도록 문구를 수정했다.
- 심사 요약서·재현 런북·포트폴리오 브리핑·하드웨어 체크리스트를 현행
  `FIT0482 X/Y + ESP32 PID + 스위치봇` 구조와 306개 테스트 기준으로 동기화했다.
- NK1704S/TB6600/GRBL 프레임 자료는 삭제하지 않고 레거시 설계 기록 배너를
  추가해 의사결정 이력은 보존하되 현재 제작 지시로 오인되지 않게 했다.

---

## 2026-09-07 — 백엔드: 수원대학교 AI API Gateway 연동 (제공자 선택형)

### 배경
- 백엔드가 google.generativeai SDK로 Gemini를 직접 호출하고 있었는데,
  학교 API Gateway(OpenAI 호환, 하나의 키로 여러 모델)를 쓸 수 있게 됐다.
  사용자가 PowerShell 직접 호출로 텍스트 채팅(gemini-3.8-flash)은 검증 완료.
- 원칙: 제공자는 환경 변수로 **명시적으로** 선택하고, 학교 API 선택 상태에서
  실패하면 Google로 조용히 우회하지 않는다.

### 변경 파일
- **신규 `backend/school_gateway.py`** — 게이트웨이 클라이언트.
  - `POST {base}/chat/completions/`, Authorization: Bearer, stream:false.
  - Base URL은 https만 허용, `allow_redirects=False`(3xx는 실패 처리 —
    인증 헤더가 다른 호스트로 전송되는 것 방지).
  - 오류 분류: config/auth(401·403)/rate_limit(429)/server(5xx)/timeout/
    network/bad_response. **재시도 없음**(크레딧 차감·60초 120건 제한 고려).
  - 타임아웃: 텍스트 25초/비전 40초(엔드포인트 전체 상한 30/45초보다 안쪽).
  - 키·사용자 발화·이미지 원문은 로그/예외 메시지에 싣지 않는다.
  - 관측 로그: `ai.call provider=school model_req=… model_resp=… status=ok
    elapsed_ms=…` — 학교 API 실사용 여부를 이 라인으로 구별한다.
- **신규 `backend/ai_provider.py`** — 제공자 해석 + 학교 경로 해석/비전.
  - `resolve_provider`: 미설정=google(기존 배포 호환). 알 수 없는 값은 기본값으로
    덮지 않고 무효 플래그 → 호출 시 설정 오류로 실패(오설정 은폐 방지).
  - `interpret_with_school`: 항상 `sanitize_command_response` 통과. 실패 시
    실행 명령 없는 안전 NONE 응답 + 원인별 한국어 안내.
  - `vision_with_school`: `sanitize_vision_mapping_response` 통과. 실패는
    SchoolGatewayError로 올려 HTTP 코드(429/502/503/504)로 매핑.
  - fastapi/genai 무의존 → 외부 키 없이 mock 단위 테스트 가능.
- **`backend/main.py`** — AI_PROVIDER 분기(텍스트·비전), 시작 시
  `ai.provider.selected` 로그(설정 불완전 시 CRITICAL), Google 경로에도
  `ai.call provider=google` 로그 추가, `GET /` 응답에 `ai_provider`·
  `ai_text_model` 노출(비밀 없음 — AI 연결 성공의 증거 아님 명시).
  간단 규칙→음식 추론→AI 순서, 레이트리밋, X-API-Key 인증은 무변경.
- **`backend/requirements.txt`** — requests==2.32.5 추가.
- **`backend/.env_ex`** — AI_PROVIDER, SCHOOL_API_KEY/BASE_URL/MODEL/
  VISION_MODEL 항목 추가(실키 없음). **README** — Render 환경 변수·재배포·
  배포 후 확인 절차 추가.

### 테스트 (로컬 mock — 외부 키/네트워크 불필요)
- 신규 `backend/test_school_gateway.py` 19건 + `backend/test_ai_provider.py`
  15건, 기존 `test_validation.py` 3건 포함 **37/37 통과**
  (`python -m unittest test_school_gateway test_ai_provider test_validation`).
- 커버: 환경 변수 처리/URL 검증(http 거부)/헤더·페이로드 구성/비전 data URI/
  한국어 UTF-8/정상 파싱+sanitizer 적용/빈 응답·비JSON·예상 밖 구조/
  401·403·429·5xx·타임아웃·리다이렉트 거부/재시도 없음/키·발화 비노출/
