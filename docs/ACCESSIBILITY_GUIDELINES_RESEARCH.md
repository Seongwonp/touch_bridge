...
> 작성일: 2026-08-09 · 출처/구현 재감사: 2026-09-04
...
> 관련: [ACCESSIBILITY_COPY_GUIDE.md](ACCESSIBILITY_COPY_GUIDE.md), [ACCESSIBILITY_REDESIGN_PLAN.md](ACCESSIBILITY_REDESIGN_PLAN.md), [REPRO_RUNBOOK.md](REPRO_RUNBOOK.md), [SOURCE_AUDIT_2026-09-04.md](SOURCE_AUDIT_2026-09-04.md)

## 목표 수준

- **WCAG 2.2 AA + 안전·저시력 관련 선택적 AAA**.
- WCAG는 원래 웹 표준이므로 네이티브 Flutter 앱에는 W3C의 **WCAG2Mobile / WCAG2ICT** 해석을 함께 적용한다.
- 국내 출시 QA에는 WCAG와 함께 **KS X 3253(모바일 앱 접근성)** 및
  **장애인차별금지법**을 검토한다. 법적 준수 여부는 최신 공식 원문과 전문가
  검토로 최종 판단한다(아래 4장).
...
- **장애인차별금지법 제21조와 관련 시행령**: 전자정보 접근 및 필요한 편의
  제공의 근거다. 법이 모든 앱에 특정한 `음성명령 UI`를 직접 요구한다고
  단정하지 않는다. Touch Bridge의 음성 제어는 여러 접근성 수단 중 하나다.
- **KS X 3253 (모바일 애플리케이션 콘텐츠 접근성 지침)**: 대체텍스트·입력
  조작·대비 등을 국내 출시 QA에 병기한다. 정확한 최신 개정일과 조항은 공식
  열람본으로 출시 전에 다시 확인한다.
...
## 6. Touch Bridge 적용 현황 & 남은 P0 (2026-09-04 기준)
...
- 앱 전체 이중 확인 대기시간 20초 통일, TTS 우선순위 큐와 화면읽기 중복 발화
  억제, 명시 버튼/접근성 액션 대안을 구현했다.
- 화면읽기 활성 시 단순 화면 이동은 앱 확인을 생략하되 물리 작동·삭제 확인은
  유지한다.
- WCAG 2.5.7에 따라 사진 매핑의 모서리/버튼 드래그에 상하좌우 조그 버튼을
  제공한다.
- 사진 촬영 전에 뒤/좌우/상하/수평·반사 확인을 음성·진동으로 안내한다.

**아직 열려 있는 P0:**

1. Android TalkBack·iOS VoiceOver 실제 기기 전체 과업 회귀.
2. 전맹·저시력 참여자 분리 모집과 실제 과업 평가.
3. FIT0482·ESP32·스위치봇 실기 안전/오차/ACK 검증.

## 7. QA 체크리스트 (요약)

전체 항목은 리서치 원문 기준. 핵심 최소 세트:
- [ ] VoiceOver/TalkBack **만으로** 기기 선택→음성 명령→확인→완료→비상정지 완주.
- [x] 코드상 주요 조작 요소에 고유 label·역할·상태와 결과 중심 hint 제공.
- [ ] 화면 전환 후 포커스가 논리적 첫 요소로 이동.
- [ ] iOS 최대 Dynamic Type / Android 200%에서 잘림·겹침 없음.
- [x] 주요 색상 쌍 AA 4.5:1(핵심 7:1) 자동 검사.
- [x] 주요 터치 영역 48×48 logical px 이상 코드 검증.
- [ ] earcon 3종(성공/실패/경고) 리듬·음고 상이, TTS와 비중첩.
- [x] 위젯 테스트에서 화면읽기 단순 이동 확인 생략·물리 동작 확인 유지 검증.
- [ ] BLE 지연·끊김·ESP32 무응답·로봇 오류 각각 원인+다음행동 안내.
- [ ] **전맹·저시력 당사자 분리 모집** 평가(성공률·오류율·시간·중도포기). 최소 기준: 도움 없이 핵심 작업 100% 완료, 안전 오류 0건.

> 체크 완료는 코드/자동 테스트 범위다. 실제 TalkBack·VoiceOver와 당사자 평가가
> 끝나기 전에는 제품 접근성 적합 판정으로 사용하지 않는다.
