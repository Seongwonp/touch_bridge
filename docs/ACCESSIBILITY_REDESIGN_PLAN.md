...
하드웨어 구성은 [HARDWARE_MIGRATION_PLAN.md](HARDWARE_MIGRATION_PLAN.md)를 따른다. 현재 기준은 X/Y/Z 전 축 NK1704S + TB6600 3개, 12V 분리 전원, Arduino Uno GRBL 유지, ESP32 BLE/향후 Wi-Fi -> UART 브릿지 구조다.

접근성 관점에서 이 구조의 책임은 다음처럼 분리한다.

- 사용자는 좌표, G-code, GRBL 상태를 알 필요가 없다. 사용자는 "전자레인지 30초" 같은 의도만 표현한다.
- 보호자는 사진 매핑, 기준점, 테스트 터치처럼 시각 확인이 필요한 초기 설정을 책임진다.
- 개발자 모드만 G-code, GRBL 로그, raw 명령, 축 조그 같은 저수준 정보를 다룬다.
- GRBL 응답(`ok`, `error`, `ALARM`)은 앱 경계에서 사람의 말로 번역되어야 하며, 사용자/보호자 화면에는 원문을 그대로 노출하지 않는다.
...
- 개발자 모드: 설정 내부에서 별도 토글로 켜며, `음성 테스트`, `BLE 로그`, `개발자 콘솔`, `ESP 선택`, `조이스틱`, `raw 명령`, `속도 조절`, `$H` 홈 복귀, Z축 테스트 누름, GRBL `ok/error/ALARM` 로그, dry-run G-code 로그 제공
- 추천 변경안: 개발자 모드 토글 자체를 릴리스 빌드에서 제거할 수 있도록 `kReleaseMode` 또는 컴파일 타임 플래그 게이트를 검토한다.
...
- `ALARM` 수신 시 즉시 "기기가 안전을 위해 스스로 멈췄습니다."라고 안내하고 비상 정지/복구 흐름으로 전환한다.
- `error` 수신 시 "실행하지 못했습니다."와 재시도 경로를 안내한다. 에러 코드 번호는 개발자 로그에만 남긴다.

**추천 변경안 (코드 미반영)**
- `hardware_protocol.dart` 또는 `mapping_execution_service.dart` 경계에서 하드웨어 응답을 성공/실패/안전정지 같은 도메인 결과로 변환하고, 화면은 도메인 결과만 받아 TTS를 출력하는 구조를 검토한다.
- `ALARM` 복구 절차(`$X` 또는 리셋 후 `$H`)는 보호자/개발자 모드 전용으로 제한한다.

**미해결 질문**
- 비상 정지 시 GRBL 처리 방식은 즉시 정지(`!` feed hold + soft reset)인지, 진행 중 시퀀스 완료 후 정지인지 하드웨어 검증이 필요하다. 안전상 즉시 정지를 권장하지만, Z축이 눌린 상태에서 멈출 때의 복구 절차를 함께 정의해야 한다.
