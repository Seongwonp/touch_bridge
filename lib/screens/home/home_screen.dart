...
class _HomeScreenState extends State<HomeScreen> {
  ...
  static final _defaultDevices = [
    DeviceInfo(
      id: 'default_microwave',
      name: '스마트 전자레인지',
      status: '작동 대기 중',
      iconCodePoint: Icons.microwave_rounded.codePoint,
    ),
    DeviceInfo(
      id: 'default_air',
      name: '스마트 공기청정기',
    ...
    DeviceInfo(
      ...
      name: '스마트 전등 허브',
      status: '원격 제어 가능',
      iconCodePoint: Icons.light_mode_rounded.codePoint,
    ),
  ];
  ...
  Future<List<DeviceInfo>> _migrateDevicesIfNeeded(
  ...
  Future<void> _announceScreen() async {
    if (_devices.isEmpty) {
      ...
      await _tts.speak(
        '홈 화면입니다. 현재 ${_devices[_currentDeviceIndex].name}, 상태: ${_devices[_currentDeviceIndex].status}. 좌우로 밀어서 기기를 전환하고, 기기를 눌러 제어하세요.',
      );
    }
  }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (!mounted) return;
                final mappingDone = await Navigator.of(context).push<bool>(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // 페이지 인디케이터
                    Semantics(
                      // 페이지 인디케이터 Semantics 추가
                      label:
                          '현재 ${_currentDeviceIndex + 1}번째 기기, 총 ${_devices.length}개 기기',
                      ...
                      Expanded(
                        child: PageView.builder(
                          ...
                          onPageChanged: (index) {
                            ...
                            _tts.speak(
                              '${_devices[index].name}, 상태: ${_devices[index].status}',
                            ); // 기기 전환 시 상세 안내
                          },
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Padding(
                              ...
                              child: Semantics(
                                // 기기 카드 Semantics 추가
                                label:
                                    '${device.name}, 상태: ${device.status}. 눌러서 제어 모드 선택. 길게 눌러서 삭제.',
                                button: true,
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: ResponsiveScale.v(context, 8)),
                          const Text(
                            '좌우로 밀어서 기기 전환',
                            style: TextStyle(
                              fontSize: 12,
...
class _ControlModeSheetState extends State<_ControlModeSheet> {
  ...
  void initState() {
    super.initState();
    _tts.speak(
      '${widget.deviceName} 제어 방식 선택 화면입니다. 원하는 버튼을 한 번 누르면 선택되고, 한 번 더 누르면 실행됩니다.',
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          Semantics(
            // 기기 이름 및 제어 방식 선택 안내 Semantics 추가
          ...
          SizedBox(
            ...
            height: 72,
            child: Semantics(
              // 음성으로 제어 버튼 Semantics 추가
              label:
                  '${widget.deviceName}을 음성으로 제어. ${_armedAction == 'voice' ? '선택됨. 한 번 더 탭하면 실행됩니다.' : ''}',
              button: true,
          ...
          SizedBox(
            ...
            height: 72,
            child: Semantics(
              // 수동으로 조작 버튼 Semantics 추가
              label:
                  '${widget.deviceName}을 수동으로 조작. ${_armedAction == 'manual' ? '선택됨. 한 번 더 탭하면 실행됩니다.' : ''}',
              button: true,
              child: ElevatedButton.icon(
                onPressed: () {
                  _armAndRun(
